...
/*
 ...
 ------------------------------------------------------------------------------------------------------------------------
 Для селекта (select):
 class="имя_класса" - модификатор к конкретному селекту. В итоге
 получится select select_имя_класса
 multiple - мультивыбор
 data-class-modif="имя_модификатора"
 data-tags - режим тегов (только для multiple)
 data-scroll - включит прокрутку для выпадающего списка,
 дополнительно можно подключить кастомный скролл simplebar в
 js/app.js. Указанное число для атрибута ограничит высоту контейнера
 выпадающего списка
 ...
 data-speed - позволяет указать скорость открытия/закрытия списка
 в миллисекундах, по умолчанию 150
 ...
 Для элемента (option):
 data-class="имя_класса" - добавляет класс
 data-asset="путь_к_картинке_или_текст" - добавляет структуру 2-х колонок и данными
 data-href="адрес_ссылки" - добавляет ссылку в элемент списка
...
import {isMobile} from '../functions/services/flags.js'
import {_slideUp, _slideDown, _slideToggle} from '../functions/services/services.js'
import {formValidate} from '../functions/forms/forms.js'
// Подключение файла стилей
// Базовые стили подключаются в src/scss/base/forms/forms.scss
// Файл базовых стилей: src/scss/base/forms/select.scss
...
class SelectConstructor {
  constructor(props, data = null) {
    ...
    this.selectClasses = {
      ...
      classSelectDisabled: '_select-disabled', // Запрещен
  ...
  selectAction(selectItem) {
    ...
    // Если селекты помещены в элемент с дата атрибутом data-one-select
