// Модуль попапов (всплывающих/модальных окон)
/*
 Открытие YouTube видео в попапе
 ------------------------------------------------------------------------------------------------------------------------
 Для того чтобы открыть видеоролик в попапе, следует добавить блоку
 попапа атрибут data-youtube, а в качестве значения указать код ролика.
 Также следует указать атрибут data-youtube-place для объекта в
 котором хотим вывести ролик.
 ------------------------------------------------------------------------------------------------------------------------

 Методы и события
 ------------------------------------------------------------------------------------------------------------------------
 Работать с попапом из любого места можно импортировав переменную modules:
 import {modules} from '../functions/services/modules.js'
 Далее обратится к классу popup и работать с методом, например open():
 modules.popup.open('#popup')
...
// Файл базовых стилей: src/scss/base/popup.scss
  ...
  constructor(options) {
    let config = {
      ...
      classes: {
        ...
        bodyActive: 'popup-show', // Добавляется для body, когда попап открыт
      },
